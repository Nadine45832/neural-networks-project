...
print(
    df[
        ["First_Term_GPA", "Second_Term_GPA", "HS_Average", "Math_Score"]
    ].describe()
)
num_cols = ["First_Term_GPA", "Second_Term_GPA", "HS_Average", "Math_Score"]
...
df["Second_Term_GPA"] = df["Second_Term_GPA"].fillna(
    df["Second_Term_GPA"].median()
)
...
param_grid = {
    "classifier__hidden_layer_sizes": [(64, 32), (128, 32), (32, 16)],
...
nn_model = keras.Sequential(
    [
        layers.Dense(
            256, activation="relu", input_shape=(X_train_nn.shape[1],)
        ),
        layers.Dropout(0.4),
...
callbacks = [
    ...
    keras.callbacks.ModelCheckpoint(
        "best_rf_nn_model.keras", save_best_only=True
    ),
]
...
joblib.dump(rf, r"rf_leaf_model.pkl")

# exporting leaf encoder
joblib.dump(encoder, r"rf_leaf_encoder.pkl")

# exporting preprocessor (ColumnTransformer)
joblib.dump(preprocessor, r"preprocessor.pkl")
...
print(
    "Hybrid RF+NN components saved: rf_leaf_model.pkl, rf_leaf_encoder.pkl, preprocessor.pkl, student_persistence_hybrid_rf_nn.keras"
)
